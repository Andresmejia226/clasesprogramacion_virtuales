...
namespace clasesvirtualesprogramacion.Forms
{
    public partial class frmEstudiantesList : Form
    {
        ...
        private void btnNuevo_Click(object sender, EventArgs e)
        {
            ...
            if (frmNuevo.DialogResult == DialogResult.OK)
            {
                ...
                if(oConexion.AccionSQL(sqlInsert) == true)
                {
                    ...
                    MessageBox.Show("La informacíón de estudiantes ha sido almacenada correctamente.", "Guardar", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void btnEliminar_Click(object sender, EventArgs e)
        {
            if (estudiantesBindingSource.Count > 0)
            {
                if (MessageBox.Show("Asegurese de querer eliminar la información del estudiantes. Desea eliminar permanentemente este regisro?", "Eliminar", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
                {
                    DataGridViewRow Fila = estudiantesDataGridView.CurrentRow;
                    Int16 ID = Int16.Parse(Fila.Cells[0].Value.ToString());
        ...
        private void btnBuscar_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Debe seleccionar una de las opciones para buscar un estudiante, ya sea por identidad, por nombres o apellidos", "Buscar", MessageBoxButtons.OK, MessageBoxIcon.Information);
                cmbBuscarPor.Focus();
                return;
                ...
                else
                {
                    ...
                    switch (cmbBuscarPor.SelectedIndex)
                    {
                        case 0: // identidad
                            ...
                            break;
                        case 1: // nombres
                            sqlSelect = string.Format("Select * from estudiantes where nombres like '{1}%'", txtCriterio.Text.Trim());
                            break;
                        default: //apellidos
                            sqlSelect = string.Format("Select * from estudiantes where apellidos like '{2}%'", txtCriterio.Text.Trim());
                            break;
                    }
                    dsclasesvirtuales.Estudiantes.Clear();
